...
export default function PostDetailScreen({ route, navigation }: any) {
  ...
  const [videoMenuOpenFor, setVideoMenuOpenFor] = useState<number | null>(null);
...
const confirmAndReportPost = useCallback(() => {
  ...
  const confirmAndReport = useCallback(
    async () => {
      ...
      if (!uri) return;
      if (!token) {
        Alert.alert("Sign in required", "You need to log in to report media.");
        return;
      }
  
      // already reported?
      if (reportedByMe[uri]) {
        Alert.alert("Already reported", "You’ve already reported this media.");
        return;
      }
      ...
      try {
        ...
        Alert.alert(
          "Error",
          "Could not report this media. Please try again."
        );
      } finally {
        setReportingUri(null);
      }
    },
    [currentViewerUri, token, postId, reportedByMe]
  );
  ...
  const saveVideo = useCallback(
    async (uri: string) => {
      try {
        if (!uri || savingVideoUri) return;
  
        setSavingVideoUri(uri);
        const encoded = uri.startsWith("http") ? encodeURI(uri) : uri;
        ...
        if (
          !CameraRoll ||
          typeof (CameraRoll as any).save !== "function"
        ) {
          throw new Error(
            "CameraRoll native module not linked. Run `npx pod-install` and rebuild."
          );
        }
  
        const fname = `post_${postId}_${Date.now()}.mp4`;
        ...
        console.log("saveVideo error =>", e?.message ?? e);
        Alert.alert(
          "Save failed",
          e?.message ? String(e.message) : "Could not save the video."
        );
      } finally {
        setSavingVideoUri(null);
      }
    },
    [postId, savingVideoUri]
  );
  
  const gatedSaveVideo = useCallback(
    (uri: string) => {
      if (!uri) return;
      if (canDownload) {
        saveVideo(uri);
  ...
  const confirmAndReportVideo = useCallback(
    (videoUrl: string) => {
      ...
      if (!token) {
        Alert.alert("Sign in required", "You need to log in to report videos.");
        return;
      }
      ...
      Alert.alert("Report video", "Are you sure you want to report this video?", [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            ...
            setReportedReason((prev) => ({ ...prev, [uri]: "" }));
            try {
              // ✅ reuse same API – backend just stores the URL
              const resp = await reportImageApi(postId, uri, "", token);
              setReportedReason((prev) => ({
                ...prev,
                [uri]: resp?.reason || "",
              ...
              // Alert.alert(
              //   "Error",
              //   "Could not report this video. Please try again."
              // );
            } finally {
              setReportingUri(null);
            }
                  ...
                  return (
                    <View key={`${m.type}-${i}`} style={{ width: SCREEN_W }}>
                      {m.type === 'image' ? (
                        ...
                        {/* 🔹 TOP-RIGHT: 3-dot menu for Save + Report (video) */}
                        <View style={styles.videoMenuTriggerWrap}>
                          <TouchableOpacity
                            onPress={() =>
                              setVideoMenuOpenFor((prev) => (prev === i ? null : i))
                            }
                            hitSlop={hit}
                        ...
                        {videoMenuOpenFor === i && (
                          <>
                            {/* backdrop just over this media area */}
                            <TouchableOpacity
                              style={styles.menuBackdrop}
                              onPress={() => setVideoMenuOpenFor(null)}
                              activeOpacity={1}
                            />
                            ...
                            >
                              ...
                              <TouchableOpacity
                                ...
                                onPress={() => {
                                  setVideoMenuOpenFor(null);
                                  gatedSaveVideo(m.uri);
                                }}
                              >
                        ...
                        {/* existing buffering + play overlays */}
          ...
          HeaderComponent={() => {
            ...
            return (
              ...
              >
                ...
                {/* LEFT: (empty, we removed the standalone Save button) */}
                <View style={{ width: 80 }} />
                  ...
                  <>
                    ...
                    >
                      ...
                      <TouchableOpacity
                        ...
                        onPress={() => {
                          setViewerMenuOpen(false);
                          confirmAndReport(); // uses currentViewerUri
                        }}
                      >
                        <Feather name="flag" size={18} color="#ff4d4d" />
                        <Text style={[styles.menuText, { color: "#ff4d4d" }]}>
...
async function reportImageApi(postId: string, imageUrl: string, reason: string, token?: string) {
  ...
  console.log("📤 Report API URL:", `${WP_BASE}/wp-json/app/v1/report_image`);
  console.log("📤 Report API Body:", JSON.stringify(payload, null, 2));
