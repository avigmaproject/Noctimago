  ...
  return (
    <View style={styles.cRowWrap}>
      <TouchableOpacity
        ...
        activeOpacity={0.9}
      >
        <Avatar uri={c?.author_profile_image } name={c.author} size={20} border />
      </TouchableOpacity>
...
const MetaBlock = React.memo(function MetaBlock({
  ...
  const caption = (post?.content && String(post.content)) || ' ';
...
>
  ...
  <Text style={styles.actionNum}>{formatCount(commentCount)}</Text>
      ...
      <TouchableOpacity style={styles.authorRow} onPress={() => navigation.navigate('ViewProfileScreen', { NTN_User_PkeyID:post?.author_id })}>
        <Avatar uri={post?.author_profile_image } name={authorName} size={30} border />
      ...
      {!!caption?.trim() && <View style={{ marginTop: 6 }}>{renderCaption(caption)}</View>}
      ...
      <View style={{ marginTop: 14, gap: 12 }}>
        ...
        {totalComments === 0 && (
          <Text style={{ color: COLORS.sub }}>Be the first to comment.</Text>
        )}

        {totalComments > PREVIEW_ROOTS && (
          <TouchableOpacity onPress={onOpenAllComments} style={{ marginTop: 6 }}>
            <Text style={styles.linkWord}>View all {totalComments} comments</Text>
          </TouchableOpacity>
        )}
      </View>
    </View>
...
export default function PostDetailScreen({ route, navigation }: any) {
  ...
  // detect tagged
  useEffect(() => {
    const tagStr: string | undefined = post?.fields?.tag_people || post?.tag_people || '';
    if (!tagStr || !currentUserName) {
      setIsTagged(false);
      return;
    }
    const tags = String(tagStr).split(',').map((s) => s.trim().toLowerCase()).filter(Boolean);
    setIsTagged(tags.includes(currentUserName.trim().toLowerCase()));
  ...
  const fetchPost = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${WP_BASE}/wp-json/app/v1/get_post/${postId}`, { headers });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const data = await res.json();
      const p = data?.post ?? null;
      setPost(p);
console.log("ppp",token,res)
      ...
      // (we now load comments via loadComments separately)
    } catch {
  ...
  const media: MediaItem[] = useMemo(() => {
    ...
    images.forEach((uri: string) => items.push({ type: 'image', uri }));
    if (f.video) items.push({ type: 'video', uri: f.video });
    if (items.length === 0) items.push({ type: 'image', uri: PLACEHOLDER_URL });
    return items;
  }, [post]);
...
async function notifyCommentAuthorLike({
  ...
  const onSendFromModal = useCallback(async ({
    text,
    tagged,
    parentId,
  }: {
    ...
    try {
      ...
      if (post?.author_id && userprofile?.ID && Number(post.author_id) !== Number(userprofile.ID)) {
        try {
          const payload = JSON.stringify({
            UserToken: "",
            message: `${userprofile?.username || 'Someone'} commented on your post`,
            msgtitle: "New comment",
            User_PkeyID: userprofile?.ID,
            UserID: post?.author_id,
            NTN_C_L: 1,
            NTN_Sender_Name: userprofile?.meta?.first_name || userprofile?.username,
            NTN_Sender_Img: userprofile?.meta?.profile_image || userprofile?.user_image_url,
            NTN_Reciever_Name: "",
            NTN_Reciever_Img: "",
  ...
  }, [token, postId, userprofile?.ID, currentUserId, post, loadComments]);
        ...
        >
          ...
          >
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'flex-end', marginTop: 16 }}>
              ...
              >
                <Text style={{ color: COLORS.sub, fontWeight: '600' }}>Maybe later</Text>
              </TouchableOpacity>
              ...
              {!liked && (
                ...
                >
                  <Text style={styles.sendTxt}>Like now</Text>
                </TouchableOpacity>
              )}
              {liked && !hasCommented && (
                <TouchableOpacity
                  ...
                  style={[styles.sendBtn, { marginLeft: 8 }]}
                >
                  <Text style={styles.sendTxt}>Comment</Text>
                </TouchableOpacity>
              )}
              {liked && hasCommented && !hasShared && (
                <TouchableOpacity
                  ...
                  style={[styles.sendBtn, { marginLeft: 8 }]}
                >
                  <Text style={styles.sendTxt}>Share</Text>
                </TouchableOpacity>
              )}
            </View>
          </View>
      ...
      {imageSources.length > 0 && (
        <ImageView
          ...
          HeaderComponent={() => {
            ...
            const already = !!(uri && reportedByMe[uri]);
            const busy = reportingUri === uri;
            ...
            return (
              ...
              >
                <View style={{ alignItems: 'flex-start', gap: 8 }}>
                  <TouchableOpacity
                    onPress={confirmAndReport}
                    disabled={already || busy}
                    activeOpacity={0.85}
                    hitSlop={hit}
                    style={[styles.reportBtn, (already || busy) && styles.reportBtnDisabled]}
                  >
                    <Feather name="flag" size={16} color="#fff" />
                    <Text style={styles.reportTxt}>
                      {already ? 'Reported' : 'Report this image'}
                    </Text>
                  </TouchableOpacity>
                </View>
                ...
                >
                  ...
                  <Text style={styles.reportTxt}>Save</Text>
                </TouchableOpacity>

                <TouchableOpacity onPress={() => setViewerVisible(false)} hitSlop={hit} style={styles.closeBtn}>
          ...
          FooterComponent={({ imageIndex }) => (
            <View style={[styles.viewerFooter, { marginBottom: insets.bottom + 12 }]}>
              <Text style={styles.viewerFooterTxt}>
