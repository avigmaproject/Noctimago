...
import {
  View, Text, KeyboardAvoidingView, Platform, StyleSheet, StatusBar,
...
/* ---------- Theme ---------- */
const COLORS1 = { bg:'#0E0F12', border:'#1F2127', active:'#E53935', inactive:'#8B8F98' };
...
/* ---------- Layout ---------- */
const HEADER_H = 56;
const COMPOSER_H = 60; // visual height of the composer bar
...
/* ---------- Header ---------- */
const Header = ({
  title, avatar, onBack, topInset
}: { title: string; avatar?: string; onBack: () => void; topInset: number }) => (
  <View style={[styles.header, { height: HEADER_H + topInset, paddingTop: topInset }]}> 
...
export default function ChatDebug({ navigation, route }: any) {
  ...
  const pickFromGallery = async () => {
    try {
      ...
      const file = { name: `${Date.now()}-${myUid}`, size: res.size, type: res.mime, uri: res.path } as any;
  ...
  // Bottom padding for the list so bubbles never hide behind the composer
  const listBottomPadding = COMPOSER_H + (Platform.OS === "ios" ? insets.bottom : 8);

  // iOS: small offset so KAV clears the header
  const KAV_OFFSET_IOS = 10;

  return (
    <SafeAreaView style={styles.screen}>
      <StatusBar barStyle="light-content" backgroundColor={COLORS.bg} />
      <Header title={otherName} avatar={otherAvatar} onBack={() => navigation.goBack()} topInset={insets.top} />
      ...
      {!ready ? (
        ...
        //   keyboardVerticalOffset={Platform.OS === "ios" ? KAV_OFFSET_IOS : 1000}
        >
          <FlatList
            ref={listRef}
            style={{ flex: 1 }}
          ...
          {/* Composer (kept in normal flow on both platforms) */}
